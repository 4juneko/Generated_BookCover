...
def drawAnnotations(image, text_boxes, ax=None):
    if ax is None:
        _, ax = plt.subplots()

    revised_boxes = []
    for text_box in text_boxes:
        x, y, h, w = text_box.x, text_box.y, text_box.h, text_box.w
        # 각 상자의 꼭짓점 좌표를 계산합니다.
        box_coords = [
            [x, y],
            [x + w, y],
        ...
        revised_boxes.append(np.array(box_coords, dtype=np.int32))
    
    ax.imshow(drawBoxes(image=image, boxes=revised_boxes))
    boxes = revised_boxes
    left = []
    right = []
    for idx, box in enumerate(boxes, start=1):
        if box[:, 0].min() < image.shape[1] / 2:
            ...
            right.append((idx, box))
    ax.set_yticks([])
    ax.set_xticks([])
    for side, group in zip(["left", "right"], [left, right]):
        for index, (idx, box) in enumerate(group):
            y = 1 - (index / len(group))
            xy = box[0] / np.array([image.shape[1], image.shape[0]])
            xy[1] = 1 - xy[1]
            ax.annotate(
                ...
                xytext=(-0.05 if side == "left" else 1.05, y),
                xycoords="axes fraction",
                arrowprops={"arrowstyle": "->", "color": (255 / 255, 124 / 255, 0)},  # RGB 값을 전달
                color=(255 / 255, 124 / 255, 0),  # 텍스트 색상 변경
                fontsize=14,
                horizontalalignment="right" if side == "left" else "left",
            )
    img_buf = io.BytesIO()
...
def drawBoxes(image, boxes, color=(255, 124, 0), thickness=5, boxes_format="boxes"):
